...
import OptionsProject from "../../shared/components/optionsProject/optionsProject";
...
import SheetProject from "../../shared/components/sheetProject/sheetProject";
import CarouselProjects from "../../shared/components/carouselProjects/carouselProjects";
import ToolsList from "../../shared/components/toolsList/toolsList";
...
export default function DetailProject() {
  ...
  // Memoize getRelatedProjects to stabilize the function reference
  ...
  // Scroll to top and set related projects when url changes
  useEffect(() => {
    scrollTo({ top: 0 });
    setRelatedProjects(getRelatedProjects());
  }, [url, getRelatedProjects]);

  // Define when to animate the header
  useEffect(() => {
  ...
  // Define when to animate the background
  useEffect(() => {
    const timeout = setTimeout(() => {
  ...
  // Show full navbar on scroll
  useEffect(() => {
    const handleScroll = () => {
      if (heroRef.current) {
        const heroBottom = heroRef.current.getBoundingClientRect().bottom;
        setIsScrolled(heroBottom <= 0); // If hero is no longer visible, change state
      }
    };
  ...
  return (
    <section className={styles.detailProject}>
      <div
        className={`${styles.cover} ${isOptionsOpen ? styles.show : ""}`}
      ></div>
      <div
        ...
        >
          <div ref={heroRef} className={styles.hero}>
            ...
            <div>
              ...
              <div className={`${styles.cta} ${isScrolled && styles.fixed}`}>
                <Tooltip
                  text="Compartir"
                  anchorSide={window.innerWidth <= 768 ? "top" : "right"}
                  size={window.innerWidth <= 768 ? "minimal" : ""}
                >
                  <button
                    className={styles.primaryButton}
                    onClick={() => handleShare(setIsSnackbarVisible)}
                  >
                    <svg
                      width="19"
                      height="24"
          ...
          <ul className={styles.tags}>
            {project.tags.map((skill, index) => (
              <Chip tag={skill} index={index} key={index} />
            ))}
          </ul>
          <p
      ...
      />
      {isSnackbarVisible && (
        <div className={styles.snackbar}>URL copied to clipboard!</div>
      )}
      <Footer />
    </section>
  );
